 ...
 * 
 * $Id: Rendered.java,v 1.84 2007/07/16 22:04:51 pbailey Exp $ 
 * 
 */
...
public class Rendered extends AbstractElement implements TRendered {
    ...
    protected Color3f mColor = new Color3f(Color.GRAY);
 ...
 **/
    ...
    public void render() {
        ...
        if ((renderFlags & COLOR_CHANGE) == COLOR_CHANGE) {
            if (mNode instanceof HasColor) ((HasColor) mNode).setColor(mColor);
        }
        renderFlags = 0;
    }
...
*/
    ...
    public void setModelOffsetPosition(Vector3d offset) {
    	// guarantees that mNode gets built
    	getNode3D();
    	mNode.setModelOffsetPosition(offset);
    }
    public Vector3d getModelOffsetPosition() {
    	getNode3D();
    	return mNode.getModelOffsetPosition();
    }
     ...
     */
    public final TNode3D getNode3D() {
        if (mNode == null) {
            TNode3D node = null;
            if (url != null) {
                node = new Loader3DS().getTNode3D(url);
            } else {
                node = makeNode();
            }
            setNode3D(node);
    ...
    public Color4f getColor4f() {
    	Color4f c = new Color4f(mColor.x, mColor.y, mColor.z, 1.f);
    	return c;
    }
