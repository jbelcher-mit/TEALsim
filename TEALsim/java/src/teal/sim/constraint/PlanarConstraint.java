 ...
 *
 */
public class PlanarConstraint implements Constraint {
	...
	public PlanarConstraint( Vector3d nnormal ) {
		setCoefficients( nnormal);
	}

	public void setCoefficients(Vector3d nnormal) {
...
//        radial.sub(center);
//		radial.normalize();
		double effectiver = action.dot(normal);
		Vector3d reaction = new Vector3d(normal);
		reaction.scale(-effectiver);  // find the negative of the perpendicular planar force in the original action			
...
//		reaction.add(centripetal);  // now we add into this the centripetal fo
	...
	public void set( Constraint c ) {
		...
		if( c instanceof PlanarConstraint ) {
			setCoefficients(((PlanarConstraint) c).getNormal());
