 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class InverterMagneticConfigurationSciFi extends SimEM {
    ...
    CylindricalBarMagnet movingMagnet1;
    Watcher watch;
    ...
    public InverterMagneticConfigurationSciFi() {
...
// change some features of the lighting, background color, etc., from the default values, if desired
   ...
   addElement(magnet06);
        ...
        movingMagnet1 = new CylindricalBarMagnet();
        movingMagnet1.setRadius(MagnetRadius);
        //movingMagnet1.setPauliDistance(4.*MagnetRadius);
        movingMagnet1.setMass(2.);
        movingMagnet1.setMu(-fixedMuBig);
        movingMagnet1.setID("movingMagnet");
        movingMagnet1.setPickable(false);
        movingMagnet1.setColliding(true);
        movingMagnet1.setGeneratingP(true);
        // not really set here, see resetCylindricalBarMagnet
        movingMagnet1.setPosition(new Vector3d(-20.,0., 2.));
        movingMagnet1.setMoveable(true);
        movingMagnet1.setRotable(false);
        movingMagnet1.setConstrained(true);
        sccx = new SphereCollisionController(movingMagnet1);
        sccx.setRadius(MagnetRadius);
        sccx.setTolerance(0.1);
        sccx.setMode(SphereCollisionController.WALL_SPHERE);
        //movingMagnet1.addPropertyChangeListener("charge",this );
        addElement(movingMagnet1);
         
 		PlanarConstraint arc = new PlanarConstraint(new Vector3d(0.,1.,0.));
		movingMagnet.addConstraint(arc);
		movingMagnet1.addConstraint(arc);
...
//            fmanager.addFieldLine(fl);
        ...
        for (int j = 0; j < numberFLA; j++) {
            RelativeFLine fl = new RelativeFLine(movingMagnet1, ((j ) / (numberFLA*1.)) *2.* Math.PI * 2.,.5 * Math.PI ,startFL*.8);
            fl.setType(Field.B_FIELD);
            fl.setKMax(maxStep);
  //          fmanager.addFieldLine(fl);
        }
        ...
        for (int j = 0; j < numberFLA; j++) {
            RelativeFLine fl = new RelativeFLine(movingMagnet1, ((j ) / (numberFLA*1.)) *2.* Math.PI * 2.,.5 * Math.PI ,startFL*.5);
            fl.setType(Field.B_FIELD);
            fl.setKMax(maxStep);
            fmanager.addFieldLine(fl);
        }
        ...
        numberFLA = 15;
        for (int j = 0; j < numberFLA; j++) {
            RelativeFLine fl = new RelativeFLine(movingMagnet1, ((j ) / (numberFLA*1.)) *2.* Math.PI * 2.,.5 * Math.PI ,startFL*1.);
            fl.setType(Field.B_FIELD);
            fl.setKMax(maxStep);
//            fmanager.addFieldLine(fl);
        }
...
//         maxStep=300;   
    ...
    private void resetCylindricalBarMagnet(double heightSupport, double lengthPendulum) {
        ...
        movingMagnet.setPosition(new Vector3d(20., 0., 3.));
        movingMagnet.setDirection(new Vector3d(0,1, 0));
        movingMagnet1.setPosition(new Vector3d(0., 20., 3.));
        movingMagnet1.setDirection(new Vector3d(0,1, 0));
    }
