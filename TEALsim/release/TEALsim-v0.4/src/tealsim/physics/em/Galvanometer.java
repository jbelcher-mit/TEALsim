 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class Galvanometer extends SimEM {
...
// change some features of the lighting, background color, etc., from the default values, if desired
        ...
        HelmholtzCoilLeft.setID("HelmholtzCoilRight");
...
//		rotatingCoil.addConstraint(arc);
        ...
        for (int j = 0; j < numberFLA; j++) {
            RelativeFLine fl = new RelativeFLine(rotatingCoil, ((j ) / (numberFLA*1.)) *2.* Math.PI * 2.,.5 * Math.PI ,startFL*.4);
     ...
     //       fmanager.addFieldLine(fl);
        ...
        for (int j = 0; j < numberFLA; j++) {
            RelativeFLine fl = new RelativeFLine(rotatingCoil, ((j ) / (numberFLA*1.)) *2.* Math.PI * 2.,.5 * Math.PI ,startFL*.8);
            fl.setType(Field.B_FIELD);
            fl.setKMax(maxStep);
            fmanager.addFieldLine(fl);
        }
