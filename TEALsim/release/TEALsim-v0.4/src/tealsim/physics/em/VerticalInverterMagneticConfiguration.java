 ...
 * Window&gt;Preferences&gt;Java&gt;Code Generation&gt;Code and Comments
 */
public class VerticalInverterMagneticConfiguration extends SimEM {
    ...
    public VerticalInverterMagneticConfiguration() {
        ...
        Wall wall = new Wall(new Vector3d(0., -10, 0.), 
        		new Vector3d(10., 0, 0.), new Vector3d(0., 0., 10.));
        wall.setElasticity(1.);
        addElement(wall);

        Wall wall1 = new Wall(new Vector3d(0., -3, 0.), 
        		new Vector3d(10., 0., 0.), new Vector3d(0., 0., 10.));
...
// change some features of the lighting, background color, etc., from the default values, if desired
        ...
        magnet01.setPosition(new Vector3d(0,  -2 ,0));
...
//     addElement(magnet04);
        ...
        movingMagnet.setPosition(new Vector3d(0.,-4., 0.));
        ...
        SphereCollisionController sccx2 = 
            	new SphereCollisionController(movingMagnet);
            sccx2.setRadius(.1);
            sccx2.setTolerance(0.01);
            movingMagnet.setColliding(true);
            movingMagnet.setCollisionController(sccx); 
            
        addElement(movingMagnet);
         
 		PlanarConstraint arc = new PlanarConstraint(new Vector3d(1.,0.,0.));
...
for (int j = 0; j < numberFLA; j++) {
    ...
    numberFLA = 5;
    for (int j = 0; j < numberFLA; j++) {
        RelativeFLine fl = new RelativeFLine(magnet04, ((j ) / (numberFLA*1.)) *2.* Math.PI * 2.,.5 * Math.PI ,startFL*fract);
        fl.setType(Field.B_FIELD);
        fl.setKMax(maxStep);
       fmanager.addFieldLine(fl);
    }
    ...
    private void resetCylindricalBarMagnet(double heightSupport, double lengthPendulum) {
        ...
        movingMagnet.setPosition(new Vector3d(0, -8, 0.));
